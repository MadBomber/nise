...
ISE_Client_Logging_Handler::ISE_Client_Logging_Handler (ACE_HANDLE output_handle)
...
int
ISE_Client_Logging_Handler::handle_signal (int signum,
		siginfo_t *,
		ucontext_t *)
{
	if (signum == SIGPIPE)
...
int
ISE_Client_Logging_Handler::open (void *)
{
...
/* VIRTUAL */ ACE_HANDLE
ISE_Client_Logging_Handler::get_handle (void) const
{
	ACE_TRACE ("ISE_Client_Logging_Handler::get_handle");
...
int
ISE_Client_Logging_Handler::handle_input (ACE_HANDLE handle)
{
#if 0
	ACE_DEBUG ((LM_DEBUG,
			ACE_TEXT ("in ISE_Client_Logging_Handler::handle_input, handle = %u\n"),
			handle));
#endif /* 0 */
...
int
ISE_Client_Logging_Handler::handle_exception (ACE_HANDLE handle)
{
...
int
ISE_Client_Logging_Handler::close (u_long)
{
...
int
ISE_Client_Logging_Handler::handle_output (ACE_HANDLE)
{
	return 0;
}
...
int
ISE_Client_Logging_Handler::send (ACE_Log_Record &log_record)
{
#if 0
...
else
{
	...
	/*
        ...
        ACE_DEBUG ((LM_DEBUG,
                    "Note: Sent logging message %s successfully to Server Logging Daemon!\n",
...
class ISE_Client_Logging_Acceptor : public ACE_Acceptor<ISE_Client_Logging_Handler, LOGGING_ACCEPTOR>
{
	// = TITLE
...
public:
	...
	ISE_Client_Logging_Acceptor (void);
	// Default constructor.
...
protected:
	...
	virtual int make_svc_handler (ISE_Client_Logging_Handler *&sh);
...
int
ISE_Client_Logging_Acceptor::make_svc_handler (ISE_Client_Logging_Handler *&sh)
{
...
int
ISE_Client_Logging_Acceptor::info (ACE_TCHAR **strp, size_t length) const
{
...
ISE_Client_Logging_Acceptor::ISE_Client_Logging_Acceptor (void)
...
int
ISE_Client_Logging_Acceptor::init (int argc, ACE_TCHAR *argv[])
{
	...
	else
	{
		...
		if (stream.get_remote_addr (server_addr) == -1)
			ACE_ERROR_RETURN ((LM_ERROR,
					...
					-1);
		ACE_DEBUG ((LM_DEBUG,
				ACE_TEXT ("The Client Logging Daemon is now connected to Server Logging Daemon %s on port %d on handle %u\n"),
	...
	ACE_NEW_RETURN (this->handler_,
			ISE_Client_Logging_Handler (stream.get_handle ()),
			-1);
	return 0;
}

int
ISE_Client_Logging_Acceptor::parse_args (int argc, ACE_TCHAR *argv[])
{
...
#if 1
	ACE_DEBUG ((LM_DEBUG,
			ACE_TEXT ("The Client Logging Daemon will connect to Server Logging Daemon %s on port %d\n"),
			this->server_addr_.get_host_name (),
			this->server_addr_.get_port_number ()
	));
	ACE_DEBUG ((LM_DEBUG,
...
int
ISE_Client_Logging_Acceptor::suspend (void)
{
	// To be done...
	return 0;
}
...
int
ISE_Client_Logging_Acceptor::resume (void)
{
	// To be done...
	return 0;
}
...
ACE_SVC_FACTORY_DEFINE (ISE_Client_Logging_Acceptor)

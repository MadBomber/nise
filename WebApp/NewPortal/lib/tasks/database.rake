namespace :db do
  ...
  ###########################################################################################
  desc 'Show the contents of the config/database.yml file after loading into ActiveRecord.'
  task :show_config do
    
    require 'erb'
       
    a_hash = YAML::load(
          ERB.new(IO.read("#{ENV['RAILS_ROOT']}/config/database.yml")).result
             )
    
    puts "For RAILS_ENV: #{ENV['RAILS_ENV']}"
    puts "... the config/database.yml file provides this configuration:"
    pp a_hash[ENV['RAILS_ENV']]  # TDV
  ...
  task :shell => :environment do
    ...
    case config['adapter']
      ...
      when 'sqlite3'
        command << "sqlite3 "
        command << File.join(RAILS_ROOT, config['database'])
        puts command
      else

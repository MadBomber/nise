...
#define RCSID_SFMT19937_H "$Id: SFMT19937.hpp 6409 2007-11-09 21:50:26Z ckarney $"
...
namespace RandomLib {
  ...
  private:
    ...
    union {
       ...
       **********************************************************************/
      __m128i _state_v[N128];
#endif
      /**
       * 32-bit representation.
       **********************************************************************/
      u32 _state_u[N128 * 4];	// Needed for SeedToState
      /**
       * <i>width</i>-bit representation.
       **********************************************************************/
      result_type _state_w[N];
    };
    /**
  ...
  private:
	...
	ar & boost::serialization::make_nvp("rounds", _rounds  )
	  &  boost::serialization::make_nvp("state" , _state_w );
	...
	ar & boost::serialization::make_nvp("rounds", t._rounds )
	  &  boost::serialization::make_nvp("state" , t._state_w);
      ar & boost::serialization::make_nvp("checksum", _checksum);
